...
def get_tracked_cliques(exp,parameters,data_association,all_clique_feats): 
    #self.current_exp,parameters,self.data_association,self.clique_features
    #exp,self.sim_length,self.lambda_u,all_clique_feats,all_observations_dir,self.min_feats,self.max_feats,data_association
    ...
    for lm_id in all_clique_feats.keys():
        ...
        if not init_time is None:
            if c not in tracked_cliques.keys():
                '''
                print("this is c:",c)
                print("this is the number of features:",max(all_clique_feats[lm_id].keys())) 
                print("this is init_time: ",init_time)
                '''

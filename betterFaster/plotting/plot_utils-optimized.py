...
class betterFaster_plot:
    def __init__(self,n_experiments,exp,parameters,gt_car_traj,observed_clique_ids,gt_gstates,prev_lm_est_err=None,data_association=None,frame_dir=None,show_plots=False,verbose=False,plot_int_results=False):
        ...
        for file in data_assocation_files:
            ...
            if os.path.exists(os.path.join(results_dir,"data_association/" + file)):
            ...
            '''
                ...
                print("results_dir: ",os.path.join(results_dir,"data_association/"))
                print("data_association_files: ",data_assocation_files)
                print("file: ",file) 
        ...
        if exp in all_data_associations.keys():
            self.lm_colors = generate_random_colors(all_data_associations[exp].shape[0]) 
        elif exp + 1 in all_data_associations.keys():
            self.lm_colors = generate_random_colors(all_data_associations[exp + 1].shape[0])  
        else:
            raise OSError 
    ...
    def plot_landmark_estimates(self,slam): 
        for k in range(slam.n):
            ...
            for j,lm in enumerate(particle_k.landmarks):  
                #print("len(self.lm_colors): ",self.lm_colors) 
                if not np.all(lm.mu) == 0:
                    self.ax.scatter(lm.mu[0],lm.mu[1],color=self.lm_colors[j]) 
    ...
    def plot_observations(self,robot_pose,observations_t): 
        ...
        for id_ in observed_clique_ids: 
            ...
            for obs in observations_id:
                ...
                observation_x = robot_pose[0] + obs["range"]*np.cos(obs["bearing"]) 
                observation_y = robot_pose[1] + obs["range"]*np.sin(obs["bearing"])
                if id_ in self.gt_trees[:,0]:

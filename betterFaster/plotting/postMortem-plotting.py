...
'''
out_dir = "/media/kristen/easystore1/BetterFaster/kitti_carla_simulator/exp_results/postProcessing_results"
posterior_dir = os.path.join(out_dir,"posteriors")  
# plot posteriors
...
with open(os.path.join(out_dir,"int_results/exp"+str(experiment_no)+"_int_performance_results999.pickle"),"rb") as handle:
...
axs[0].plot(np.arange((experiment_no+1)*sim_length),best_traj_estimate[:,1],'r')
axs[0].set_title('Trajectory Error',ha='left',va='center',x=-0.2,y=0.5,rotation=90)
axs[0].set_xlabel('Time')
axs[0].set_ylabel('MSE (m)')
axs[0].set_xlim(0,(experiment_no+1)*sim_length) 
...
for exp in range(experiment_no + 1): 
    with open(os.path.join(out_dir,"int_results/exp"+str(exp)+"_int_performance_results999.pickle"),"rb") as handle:
        perf_results = pickle.load(handle)
...
'''
axs[1].plot(np.arange(sim_length*(experiment_no+1)),perf_results["landmark_estimate_error_cache"]["mean"],'b')
